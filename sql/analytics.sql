...
SELECT co.country_name_fr, c.category_level2, AVG(f.sugars_100g) AS avg_sugars_100g
FROM fact_nutrition_snapshot f
JOIN dim_product p ON f.product_sk = p.product_sk
...
JOIN bridge_product_category bc ON p.product_sk = bc.product_sk
JOIN dim_category c ON bc.category_sk = c.category_sk
GROUP BY co.country_name_fr, c.category_level2
ORDER BY co.country_name_fr, c.category_level2;
...
SELECT p.code, p.product_name, f.salt_100g, f.sugars_100g
FROM fact_nutrition_snapshot f
JOIN dim_product p ON f.product_sk = p.product_sk
WHERE (f.salt_100g > 25) OR (f.sugars_100g > 80)
ORDER BY f.salt_100g DESC, f.sugars_100g DESC;

SELECT t.year, t.iso_week, ROUND(AVG(f.completeness_score), 4) AS completeness_avg, COUNT(*) AS rows_count
FROM fact_nutrition_snapshot f
